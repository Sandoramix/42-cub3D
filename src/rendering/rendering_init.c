...
{
	const t_dpoint init_ray = (t_dpoint){(game->player.x_px / 64 - game->engine.map_coords.x) * game->engine.delta_dist.x, 0};
	const t_dpoint init_ray1 = (t_dpoint){(- game->player.x_px / 64 + game->engine.map_coords.x + 1.0 ) * game->engine.delta_dist.x, 0};
	t_dpoint init_ray2;
	t_dpoint init_ray3;
	...
	init_ray2 = (t_dpoint){game->engine.ray.x, (game->player.y_px / 64 - game->engine.map_coords.y) * game->engine.delta_dist.y};
	init_ray3 = (t_dpoint){game->engine.ray.x, ( - game->player.y_px / 64 + game->engine.map_coords.y + 1.0) * game->engine.delta_dist.y};
