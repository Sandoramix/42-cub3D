...
void init_render(t_var *game, int x)
{
	...
	const double diry = game->player.dir_y + game->engine.plane.y * game->engine.camera_x;
	...
	game->engine.camera_x = (double)(2.0 * x) / (double)game->config.win_width - 1;
