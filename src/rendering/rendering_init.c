...
{
	const t_dpoint ray = (t_dpoint){(game->player.x_px / 64 - game->dda.map.x) * game->dda.delta_dist.x, 0};
	const t_dpoint ray1 = (t_dpoint){(game->dda.map.x + 1.0 - game->player.x_px / 64) * game->dda.delta_dist.x, 0};
	t_dpoint ray2;
	t_dpoint ray3;
	...
	ray2 = (t_dpoint){game->dda.ray.x,(game->player.y_px / 64 - game->dda.map.y) * game->dda.delta_dist.y};
	ray3 = (t_dpoint){game->dda.ray.x, (game->dda.map.y + 1.0 - game->player.y_px / 64) * game->dda.delta_dist.y};
	if (game->dda.dir.y < 0)
	{
		game->dda.step_y = -1;
...
void init_rendering(t_var *game, int pixel_pos_x)
{
	...
	copy_pos( &game->dda.map, playerPos);
